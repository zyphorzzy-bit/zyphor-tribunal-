const { 
  ...
  TextInputStyle, ActivityType, AttachmentBuilder, MessageType 
} = require('discord.js');
...
function atualizarPresenca(client) {
  const statusTexto = config.atendimentoAtivo ? 'ATENDIMENTO ON' : 'ATENDIMENTO OFF';
  client.user.setPresence({
    activities: [{ name: statusTexto, type: ActivityType.Streaming, url: 'https://twitch.tv/discord' }],
    status: 'online'
  });
}
...
function isStaff(member) {
  ...
  if (config.roleJuiz && member.roles.cache.has(config.roleJuiz)) return true;
  if (config.superiorRoles && member.roles.cache.some(r => config.superiorRoles.includes(r.id))) return true;
  return false;
}
...
// Remove mensagens do sistema de entrada e de membros adicionados ao tópico/ticket
...
client.on(Events.MessageCreate, async m => {
  ...
  if (cmd === 'f' && db.tickets[m.channel.id]) {
    ...
    const isOwnerTicket = m.author.id === t.dono;

    if (!isOwnerTicket && !isStaff(m.member)) {
      return m.reply(`${E.proibido} Você não tem permissão para encerrar este atendimento!`);
    }
...
client.on(Events.InteractionCreate, async int => {
  if (int.isChatInputCommand()) {
    if (int.commandName === 'config') {
      ...
      const rowTarget = new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder().setCustomId('cfg_target_chan').setPlaceholder('Canal de Tickets'));
      const rowRoleJuiz = new ActionRowBuilder().addComponents(new RoleSelectMenuBuilder().setCustomId('cfg_role_juiz').setPlaceholder('Cargo dos JUIZES'));
      ...
      const rowAdv1 = new ActionRowBuilder().addComponents(new RoleSelectMenuBuilder().setCustomId('cfg_adv1').setPlaceholder('Cargo para ADV 1'));

      return int.reply({ embeds: [embedCfg], components: [rowBtns, rowTarget, rowRoleJuiz, rowRoles, rowAdv1], ephemeral: true });
    }
  ...
  if (int.isButton() && int.customId === 'cfg_view') {
    ...
    const a1 = config.roleAdv1 ? `<@&${config.roleAdv1}>` : 'Não setado';
    const a2 = config.roleAdv2 ? `<@&${config.roleAdv2}>` : 'Não setado';
    const a3 = config.roleAdv3 ? `<@&${config.roleAdv3}>` : 'Não setado';

    return int.reply({ content: `**Configurações:**\n• **Cargo Juiz:** ${juizText}\n• **Superiores:** ${rolesText}\n• **ADV 1:** ${a1}\n• **ADV 2:** ${a2}\n• **ADV 3:** ${a3}`, ephemeral: true });
  }
  ...
  if (int.isRoleSelectMenu()) {
    ...
    if (int.customId === 'cfg_adv1') config.roleAdv1 = int.values[0];
    if (int.customId === 'cfg_adv2') config.roleAdv2 = int.values[0];
    if (int.customId === 'cfg_adv3') config.roleAdv3 = int.values[0];
    salvarCfg();
  ...
  if (int.isButton()) {
    ...
    if (int.customId === 'assumir_ticket') {
      if (!t) return;
      if (t.atendido) return int.reply({ content: `${E.proibido} Este ticket já foi assumido!`, ephemeral: true });

      if (int.user.id === t.dono) {
        return int.reply({ content: `${E.proibido} Você é o autor deste ticket e não pode assumi-lo!`, ephemeral: true });
      }

      if (!isStaff(int.member)) {
        return int.reply({ content: `${E.proibido} Apenas Juízes e membros autorizados podem assumir atendimentos!`, ephemeral: true });
      }
    ...
    if (int.customId === 'fechar_ticket') {
      if (!t) return;
      const isOwnerTicket = int.user.id === t.dono;

      if (!isOwnerTicket && !isStaff(int.member)) {
        return int.reply({ content: `${E.proibido} Você não tem permissão para fechar este atendimento!`, ephemeral: true });
      }
