...
// Função para checar se o membro é Staff (Dono, Juiz ou Superior)
...
client.on(Events.MessageCreate, async m => {
  if (m.type === MessageType.RecipientAdd || m.type === MessageType.UserJoin) {
...
client.on(Events.MessageCreate, async m => {
  ...
  if (cmd === 'f' && db.tickets[m.channel.id]) {
    if (!isStaff(m.member)) {
      return m.reply(`${E.proibido} Apenas Juízes e Superiores podem encerrar o atendimento!`);
    }
...
client.on(Events.InteractionCreate, async int => {
  ...
  if (int.isButton()) {
    ...
    if (int.customId === 'fechar_ticket') {
      if (!isStaff(int.member)) {
        return int.reply({ content: `${E.proibido} Apenas Juízes e Superiores podem fechar o ticket!`, ephemeral: true });
      }
